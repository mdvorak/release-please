...
export class Java extends BaseStrategy {
  ...
  async buildReleasePullRequest(
    ...
    if (await this.needsSnapshot(latestRelease)) {
  ...
  protected async needsSnapshot(latestRelease?: Release): Promise<boolean> {
    const version = latestRelease?.tag?.version;
    return Promise.resolve(
      version !== undefined &&
        version.toString() !== '0.0.0' &&
        (!version.preRelease || version.preRelease.indexOf('SNAPSHOT') < 0)
    );
  }
